...
#Excepciones
class ErrorMes(Exception):
    pass

class ErrorAnio(Exception):
    pass
...
class TarjetaCredito(MetodoPago):
    def __init__(self):
        self.__numero=0
        self.__mesExpiracion=0
        self.__anioExpiracion=0
        self.__codigo=0
    ...
    def procesar_pago(self, monto):
        if monto <= self.__recursos:
            try:
                ...
                if len(str(self.__numero)) != 12:
                    raise Exception("Tarjeta debe tener 12 digitos")
                self.__codigo = int(input("Ingrese el codigo de la tarjeta: "))
                self.__mesExpiracion = int(input("Ingrese el mes de expiracion: "))
                if self.__mesExpiracion<1 or self.__mesExpiracion>12:
                    raise ErrorMes("Mes Invalido")
                self.__anioExpiracion = int(input("Ingrese el anio de expiracion: "))
                if self.__anioExpiracion<26:
                    raise ErrorAnio("Tarjeta Vencida")
                self.__recursos = self.__recursos - monto
                self.pagar(monto)
                print(f"Su saldo es: {self.__recursos}")
            except ErrorMes as m1:
                print(f"Error: {m1}")
            except ErrorAnio as m2:
                print(f"Error: {m2}")
            except ValueError:
    ...
    while True:
        try:
            ...
            while True:
             ...
             print("El monto solo puede contener numeros")
Menu()
